...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        $grid = $("#gridblood");
        $grid.jqGrid({
            url:'<?php echo base_url()?>Blood/grid',
            datatype: "json",
            height: 250,
            autowidth: true,
            colNames:[
            ...
            colModel:[
                ...
                {name:'bloodid', index:'bloodid',width:130, fixed:true, searchoptions:{sopt:['cn']}},
                {name:'bloodname', index:'bloodname',width:130, fixed:true, searchoptions:{sopt:['cn']}},
            ...
            viewrecords: true,
            sortable: true,
            editurl: "<?php echo base_url()?>blood/crud",
            caption: "Data Blood",
            altRows:true,
        ...
        $grid.navButtonAdd('#pgridblood',{
            caption:"Delete&nbsp;&nbsp;", 
            title : "Del",
            id:"delblood",
            buttonicon:"ui-icon-trash", 
            onClickButton: function(){ 
                var bloodid = jQuery("#gridblood").jqGrid('getGridParam','selrow');
                if(bloodid != null){
                    del("del",bloodid);
                }
                else{
        ...
        if(acl.substr(0,1)==0){//disable view
            $('#viewblood').addClass('ui-state-disabled');
        }
        if(acl.substr(1,1)==0){//disable add
            $('#addblood').addClass('ui-state-disabled');
        }
        if(acl.substr(2,1)==0){//disable edit
            $('#editblood').addClass('ui-state-disabled');
        }
        if(acl.substr(3,1)==0){//disable del
            $('#delblood').addClass('ui-state-disabled');
        }
        if(acl.substr(6,1)==0){//disable export
            $('#excelblood').addClass('ui-state-disabled');
        }
        ...
        $("#resetFilterOptions").click(function(){
            ...
            $("#gridblood").jqGrid('setGridParam', { search: false, postData: { "filters": ""} }).trigger("reloadGrid");
        });
    });
...
function view(form,id){
    page="<?php echo base_url(); ?>blood/form/"+form+"/"+id+"/null";
    ...
    $("#formInput").dialog({
        ...
        height:250,
        modal:false,
        title:"<img class='icon' src='<?php echo base_url(); ?>libraries/icon/24x24/edit.png'><ul class='title'>Edit Data</ul>",
        buttons:[{
...
function save(form,id){
    page="<?php echo base_url(); ?>blood/form/"+form+"/"+id;
    $('#formInput').html('<img src="<?php echo base_url(); ?>libraries/img/loading.gif">').load(page);
    var opr = form;
    if(opr=="add"){
    ...
    $("#formInput").dialog({
        ...
        buttons:[{
            ...
            click:function(){
                if($("#bloodid").val()==""){
                    $("#bloodid").css("background-color","rgb(255,128,192)");
...
<div class="easyui-tabs" style="height:auto">
    <div title="Data blood" style="padding:10px">
        ...
        <table id="gridblood"></table>
        <div id="pgridblood"></div>      
