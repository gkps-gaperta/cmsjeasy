<script type="text/javascript">
    ...
    $(document).ready(function(){
        $grid = $("#gridjemaat");
        $grid.jqGrid({
            url:'<?php echo base_url()?>tb/grid/<?php echo @$statusid ?>/<?php echo @$acl; ?>',
            datatype: "json",
            height: 250,
            autowidth: true,
            colNames:[
            ...
            viewrecords: true,
            sortable: true,
            editurl: "<?php echo base_url()?>tb/crud",
            caption: "Data Teman Baru",
            altRows:true,
        ...
        .navButtonAdd('#pgridjemaat',{
           ...
           onClickButton: function(){ 
                $.ajax({
                    url:"<?php echo base_url(); ?>tb/creatrelation/",
    ...
    $(document).ready(function(){
        ...
        $("#selectboxid").live('click',function(){
            if($(this).is(":checked")){
                var recno = ($(this).val());
                return $.ajax({
                    url:"<?php echo base_url(); ?>tb/simpan_relation/"+recno,
                    success:function(data)
                    {
                    ...
                    return $.ajax({
                        ...
                        success: function(data) {
                            if(data.status=='sukses' && data.photofile!="") {
                                ...
                                $.ajaxFileUpload({
                                    url: "<?php echo base_url(); ?>tb/upload/"+data.photofile,
                                    secureuri: false,
...
<div id="tt" class="easyui-tabs">
    <div data-options="selected:true" title="Data Teman Baru" style="padding:10px">
    ...
    <div title="Create Relation" style="padding:10px">
        <?php $this->load->view("tb/gridcreaterelasi"); ?>
    </div>
</div>
