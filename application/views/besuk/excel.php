...
class Excel{
    ...
    $excel->writelabel(0, 2, "besukdate"); 
    $excel->writelabel(0, 3, "pembesuk"); 
    $excel->writelabel(0, 4, "pembesukdari"); 
    $excel->writelabel(0, 5, "remark"); 
    $excel->writelabel(0, 6, "besuklanjutan"); 
    ...
    foreach ($sql->result() as $row){
        ...
        $excel->writelabel($i, 2,$row->besukdate); 
        $excel->writelabel($i, 3,$row->pembesuk); 
        $excel->writelabel($i, 4,$row->pembesukdari); 
        $excel->writelabel($i, 5,$row->remark); 
        $excel->writelabel($i, 6,$row->besuklanjutan); 
