...
class Persekutuan extends CI_Controller {
	...
	function index(){
		$data['acl'] = $this->hakakses('persekutuan');
		$data['sqlmenu'] = $this->mmenutop->get_data();
		$this->load->view('header');
		$this->load->view('navbar',$data);
		$this->load->view('persekutuan/gridpersekutuan');
		$this->load->view('footer');
	}
	
	function grid(){
		$acl = $this->hakakses('persekutuan');
		@$page = $_POST['page']; 
		@$limit = $_POST['rows']; 
		@$sidx = $_POST['sidx']; 
		@$sord = $_POST['sord']; 
		...
		$sql = $this->mpersekutuan->count($where);
		$count = $sql->num_rows();
		if ($count > 0) {
		...
		if ($start < 0)
		    @$start = 0;
		$data = $this->mpersekutuan->get($where, $sidx, $sord, $limit, $start);
		...
		foreach($data->result() as $row){
			if(substr($acl,0,1)==1){
				$view='<a href="#" id='.$row->persekutuanid.' title="view" class="btnview" style="float:left"><span class="ui-icon ui-icon-document"></span></a>';
			}
			else{
				$view='<span style="float:left" class="ui-state-disabled ui-icon ui-icon-document"></span>';
			}
			if(substr($acl,2,1)==1){
				$edit='<a href="#" id='.$row->persekutuanid.' title="Edit" class="btnedit" style="float:left"><span class="ui-icon ui-icon-pencil"></span></a>';
			}
			else{
				$edit='<span style="float:left" class="ui-state-disabled ui-icon ui-icon-pencil"></span>';
			}
			if(substr($acl,3,1)==1){
				$del='<a href="#" id='.$row->persekutuanid.' title="Del" class="btndel" style="float:left"><span class="ui-icon ui-icon-trash"></span></a>';
			}
			else{
				$del='<span class="ui-state-disabled ui-icon ui-icon-trash"></span>';
			}
			$responce->rows[$i]['id']   = $row->persekutuanid;
	...
	function form($form,$persekutuanid){
		$data["persekutuanid"] = $persekutuanid;
		$this->load->view('persekutuan/'.$form,$data);
	}
	...
	function crud(){
		...
		@$data = array(
			'persekutuanid' => @$_POST['persekutuanid'],
			'persekutuanname' => @$_POST['persekutuanname'],
			'modifiedby' => $_SESSION['username'],
			'modifiedon' => date("Y-m-d H:i:s")
			);
	    switch ($oper) {
	        case 'add':
				$this->mpersekutuan->add("tblpersekutuan",$data);
				$hasil = array(
			        'status' => 'sukses'
			    );
	            ...
	            break;
	        case 'edit':
				$this->mpersekutuan->edit("tblpersekutuan",$data,$persekutuanid);
				$hasil = array(
			        'status' => 'sukses'
			    );
			    echo json_encode($hasil);
	            break;
	         case 'del':
				$this->mpersekutuan->del("tblpersekutuan",$persekutuanid);
				$hasil = array(
			        'status' => 'sukses'
			    );
			    echo json_encode($hasil);
	...
	function excel(){
		...
		FROM tblpersekutuan " . $where . " ORDER BY $sidx $sord");
		$this->load->view('persekutuan/excel',$data);
	}
