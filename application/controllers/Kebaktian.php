...
class Kebaktian extends CI_Controller {
	...
	function grid(){
		...
		$sql = $this->mkebaktian->count($where);
		$count = $sql->num_rows();
		if ($count > 0) {
		...
		if ($start < 0)
		    @$start = 0;
		$data = $this->mkebaktian->get($where, $sidx, $sord, $limit, $start);
		...
		foreach($data->result() as $row){
			if(substr($acl,0,1)==1){
				$view='<a href="#" id='.$row->kebaktianid.' title="view" class="btnview" style="float:left"><span class="ui-icon ui-icon-document"></span></a>';
			}
			else{
				$view='<span style="float:left" class="ui-state-disabled ui-icon ui-icon-document"></span>';
			}
			if(substr($acl,2,1)==1){
				$edit='<a href="#" id='.$row->kebaktianid.' title="Edit" class="btnedit" style="float:left"><span class="ui-icon ui-icon-pencil"></span></a>';
			}
			else{
				$edit='<span style="float:left" class="ui-state-disabled ui-icon ui-icon-pencil"></span>';
			}
			if(substr($acl,3,1)==1){
				$del='<a href="#" id='.$row->kebaktianid.' title="Del" class="btndel" style="float:left"><span class="ui-icon ui-icon-trash"></span></a>';
			}
			else{
				$del='<span class="ui-state-disabled ui-icon ui-icon-trash"></span>';
			}
			$responce->rows[$i]['id']   = $row->kebaktianid;
	...
	function form($form,$kebaktianid){
		$data["kebaktianid"] = $kebaktianid;
		$this->load->view('kebaktian/'.$form,$data);
	}
	...
	function crud(){
		...
		@$data = array(
			'kebaktianid' => @$_POST['kebaktianid'],
			'kebaktianname' => @$_POST['kebaktianname'],
			'modifiedby' => $_SESSION['username'],
			'modifiedon' => date("Y-m-d H:i:s")
			);
	    switch ($oper) {
	        case 'add':
				$this->mkebaktian->add("tblkebaktian",$data);
				$hasil = array(
			        'status' => 'sukses'
			    );
	            ...
	            break;
	        case 'edit':
				$this->mkebaktian->edit("tblkebaktian",$data,$kebaktianid);
				$hasil = array(
			        'status' => 'sukses'
			    );
			    echo json_encode($hasil);
	            break;
	         case 'del':
				$this->mkebaktian->del("tblkebaktian",$kebaktianid);
				$hasil = array(
			        'status' => 'sukses'
			    );
			    echo json_encode($hasil);
