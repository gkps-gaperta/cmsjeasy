...
class Jemaat extends CI_Controller {
	...
	function grid(){
		...
		foreach($data->result() as $row){
			...
			$rel="";
			if(mysql_num_rows(mysql_query("SHOW TABLES LIKE 'tbltemp".$_SESSION['userpk']."'"))==1){
				$tabel = "tbltemp".$_SESSION['userpk'];
				$q = mysql_query("SELECT recno FROM $tabel WHERE recno='$row->recno'");
				if($cek = mysql_fetch_array($q)){
					$rel = "checked";
				}
			}
			else{
			...
			$q = mysql_query("SELECT * FROM tblbesuk WHERE recno='$recno' ORDER BY modifiedon DESC");
			if($dta = mysql_fetch_array($q)){
				//$dta = "checked";
